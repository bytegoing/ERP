    ...
    public partial class OutForm : Form
    {
        ...
        private void OutForm_Load(object sender, EventArgs e)
        {
            ...
            if (OutterNo != string.Empty)
            {
                ...
                outCountText.Text = Outter.OutCount + "";
                outSinglePriceText.Text = Outter.OutSinglePrice + "";
                outTotalPriceText.Text = Outter.InCount * Outter.OutSinglePrice + "";
        ...
        private void OutForm_Shown(object sender, EventArgs e)
        {
            if (type != "销售出库" && type != "其他出库")
            {
        ...
        private void 保存SToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                new StockManager().Out(goodNoText.Text, warehouseNoText.Text, typeText.Text, reasonText.Text, outCountText.Text, outSinglePriceText.Text, businessUnitNoText.Text, CommonData.nowLoginUser.LoginName, date.Value);
            }
            catch (Exception exp)
            {
                MsgBox.ShowError("出库失败! " + exp.Message);
                return;
            }
            MsgBox.ShowInfo("出库成功!");
            ifChanged = false;
        }
        ...
        {
            string singlePrice = outSinglePriceText.Text;
            int singlePriceNum = 0;
            int countNum = 0;
            try
            {
                singlePriceNum = StockManager.GetValue(singlePrice);
                countNum = Convert.ToInt32(outCountText.Text);
            }
            catch (Exception)
            {
                outTotalPriceText.Text = "";
                return;
            }
            outTotalPriceText.Text = ((countNum * singlePriceNum) / 100.0).ToString();
        }

        private void OutCountText_TextChanged(object sender, EventArgs e)
        {
            outCountText.Text = outCountText.Text.Replace(".", string.Empty);
            ifChanged = true;
            TryCalc();
        }

        private void OutSinglePriceText_TextChanged(object sender, EventArgs e)
        {
            ifChanged = true;
            TryCalc();
        }
