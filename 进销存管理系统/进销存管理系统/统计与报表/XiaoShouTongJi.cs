...
{
    public partial class XiaoShouTongJi : Form
    {
        DateTime from, to;

        public XiaoShouTongJi()
        {
            InitializeComponent();
                ...
                for (int i = 0; i < list.Count; i++)
                {
                    if (list[i].Date >= from && list[i].Date <= to)
                    {
                        ...
                        if (list[i].Type == "销售出库" && (list[i].Reason == null || list[i].Reason == ""))
                        {
                            ...
                            {
                                //有的话就再加上吧
                                count[string.Format("{0}年{1}月", list[i].Date.Year, list[i].Date.Month)] += list[i].OutCount;
                            }
                            else
                            {
                                //没有就新建
                                count.Add(string.Format("{0}年{1}月", list[i].Date.Year, list[i].Date.Month), list[i].OutCount);
                            }
                        }
                    }
                }
        ...
        private void 商品类别销售量月表SToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (GetDateTime())
            {
                try
                {
                    list = new StockManager().GetList("销售出库",
        ...
        private void 客户购买量统计月表ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            List<Stock> list = null;
            Dictionary<string, List<Series>> dict = new Dictionary<string, List<Series>>();
            if (GetDateTime())
